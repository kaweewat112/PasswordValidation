...
public class TestRunner {
    ...
    public static void main(String[] a) {
        ...
        check("password no number",PasswordValidator.validate("121315156156")==false);
        // TODO: R6 - มีช่องว่าง -> false
        check("password no space",PasswordValidator.validate(" ")==false);
        // TODO: boundary อื่นๆ ที่คุณคิดว่าจำเป็น
        check("password no ภาษาไทย",PasswordValidator.validate("ภาษาไทย")==false);

        check("password len < 8",PasswordValidator.validate("A")==false );

        check("password no spec",PasswordValidator.validate("<>?/!@#$%^&*()_+")==false);

        check("password > 20 ",PasswordValidator.validate("Adsafafasfafafafafafafafafafafafafafafafafafafafafafbgdzt231")==false );
